...
outputsd, final_sd = tf.nn.dynamic_rnn(
    mcelld,                      # cell you have chosen
    tf_xa2D_rnn,                       # input
    initial_state=init_sd,       # the initial hidden state
    time_major=False,           # False: (batch, time step, input); True: (time step, batch, input)
)

outsa2D = tf.reshape(outputsd, [-1, CELL_SIZE]) 
pred=tf.matmul(outsa2D ,w_out)+b_out
