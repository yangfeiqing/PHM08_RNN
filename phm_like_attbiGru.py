# -*- coding: utf-8 -*-
"""
Created on Fri Apr 19 16:01:27 2019
...
w_outloc=tf.Variable(tf.random_normal([CELL_SIZE,OUT_SIZE]))
b_outloc=tf.Variable(tf.random_normal([OUT_SIZE]))
w_outvar=tf.Variable(tf.random_normal([CELL_SIZE,OUT_SIZE]))
b_outvar=tf.Variable(tf.random_normal([OUT_SIZE]))
...
predloc=tf.matmul(outs2dD_con ,w_outloc)+b_outloc
predvar=tf.matmul(outs2dD_con ,w_outvar)+b_outvar
#net_outs2D = tf.layers.dense(outs2D, OUT_SIZE)
outsloc = tf.reshape(predloc, [-1, TARGET_STEP, OUT_SIZE])          # reshape back to 3D
outsvar = tf.reshape(predloc, [-1, TARGET_STEP, OUT_SIZE])     

distri=tfd.MultivariateNormalDiag(outsloc, outsvar)
likelihood = distri.log_prob(tf_ya)
#distri=tfd.Independent(tfd.Bernoulli(outs), 2)
samples =distri.mean()
...
for i in range(epoch):
    ...
    for step in range(ite):
        ...
        _, pred_, final_s_ , _loss= sess.run([train_op, samples, final_s, loss], feed_dict)     # train
        losses.append(_loss)
...
#        testite=600
    for step in range(testite):
        ...
        pred_, final_s_ , _loss= sess.run([samples, final_s, loss], feed_dict)     # train
        mse_te.append(_loss)
